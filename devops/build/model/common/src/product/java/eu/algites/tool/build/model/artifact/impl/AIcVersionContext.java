package eu.algites.tool.build.model.artifact.impl;

import eu.algites.tool.build.model.artifact.common.AIiReleaseLine;
import eu.algites.tool.build.model.artifact.common.AIiReleaseLineRevision;
import eu.algites.tool.build.model.artifact.common.AIiVersionContext;
import eu.algites.tool.build.model.artifact.common.AIiVersionQualifier;

import java.util.Objects;

public class AIcVersionContext implements AIiVersionContext {

	private AIiReleaseLine releaseLine;
	private AIiReleaseLineRevision revision;
	private AIiVersionQualifier qualifier;

	public AIcVersionContext() {
	}

	public AIcVersionContext(AIiReleaseLine releaseLine, AIiReleaseLineRevision revision, AIiVersionQualifier qualifier) {
		this.releaseLine = releaseLine;
		this.revision = revision;
		this.qualifier = qualifier;
	}

	@Override
	public AIiReleaseLine getReleaseLine() {
		return releaseLine;
	}

	public void setReleaseLine(AIiReleaseLine releaseLine) {
		this.releaseLine = releaseLine;
	}

	@Override
	public AIiReleaseLineRevision getRevision() {
		return revision;
	}

	public void setRevision(AIiReleaseLineRevision revision) {
		this.revision = revision;
	}

	@Override
	public AIiVersionQualifier getQualifier() {
		return qualifier;
	}

	public void setQualifier(AIiVersionQualifier qualifier) {
		this.qualifier = qualifier;
	}

	@Override
	public int hashCode() {
		int result = Objects.hashCode(releaseLine);
		result = 31 * result + Objects.hashCode(revision);
		result = 31 * result + Objects.hashCode(qualifier);
		return result;
	}

	@Override
	public boolean equals(final Object aO) {
		if (!(aO instanceof AIcVersionContext locthat))
			return false;

		return Objects.equals(releaseLine, locthat.releaseLine) && Objects.equals(revision, locthat.revision)
				&& Objects.equals(qualifier, locthat.qualifier);
	}

	@Override
	public String toString() {
		return "AIcVersionContext{" +
				"releaseLine=" + releaseLine +
				", revision=" + revision +
				", qualifier=" + qualifier +
				'}';
	}
}
